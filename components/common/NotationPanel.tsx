...
const NotationPanel: React.FC<NotationPanelProps> = ({ clickedNote, isSustainOn, onSustainToggle }) => {
    ...
    useEffect(() => {
        if (rendererRef.current) {
            ...
            if (clickedNote) {
                try {
                    const requiredHeight = 150 + Math.abs(clickedNote.octave - 4) * 20;
                    const vf = new Factory({ renderer: { elementId: rendererRef.current.id, width: 220, height: requiredHeight } });
                    const stave = vf.Stave({ x: 10, y: 10, width: 200 });
                    stave.addClef('treble').addTimeSignature('4/4');
                    stave.setContext(vf.getContext()).draw();

                    const noteName = clickedNote.noteName.toLowerCase();
                    const noteLetter = noteName.charAt(0);
                    const hasAccidental = noteName.includes('#');

                    const staveNote = new StaveNote({ keys: [`${noteLetter}/${clickedNote.octave}`], duration: 'w' });
    ...
    return (
        <div className="p-4 rounded-lg bg-black/20 border border-purple-400/20 h-full flex flex-col">
            <h4 className="text-xl font-bold text-gray-200 mb-2 text-center">Notation Viewer</h4>
             <div className="min-h-[200px] flex-grow flex items-center justify-center bg-white/90 rounded-md">
                {clickedNote ? (

...
const NotationPanel: React.FC<{ onSustainToggle: () => void; }> = ({ onSustainToggle }) => {
    const { clickedNote, isSustainOn } = useAppContext();
        ...
        if (rendererRef.current) {
            ...
            if (noteToRender) {
                try {
                    ...
                    const position = (octave - 4) * -7 + (4 - noteIndex);
                    const staveY = 40 - position * 5;
             ...
             <div className="min-h-[200px] flex-grow flex items-center justify-center bg-white/90 rounded-md overflow-hidden">
                {noteToRender ? (
                    ...
                    <div className="text-center text-gray-600 p-4">
                        ...
                        <p className="font-semibold">Click any note</p>
                    </div>
                )}
            </div>

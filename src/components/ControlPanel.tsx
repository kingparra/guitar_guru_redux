...
import { InfoIcon, SettingsIcon, NavIcon } from './common/Icons';

interface ControlPanelProps {
    onGenerate: (note: string, scale: string) => void;
    onSavePdf: () => void;
    isLoading: boolean;
    isSavingPdf: boolean;
    hasContent: boolean;
...
const SetupMode: React.FC<Pick<ControlPanelProps, 'onGenerate' | 'isLoading' | 'isSavingPdf' | 'onToggleAnalyzer' | 'isAnalyzerVisible'>> = 
({ onGenerate, isLoading, isSavingPdf, onToggleAnalyzer, isAnalyzerVisible }) => {
    const { rootNote, scaleName, dispatch } = useAppContext();
    const setRootNote = (note: string) => dispatch({ type: 'SET_ROOT_NOTE', payload: note });
    const setScaleName = (scale: string) => dispatch({ type: 'SET_SCALE_NAME', payload: scale });
    ...
    return (
        <>
            ...
            <button onClick={() => onGenerate(rootNote, scaleName)} disabled={isLoading} className="w-full md:w-auto bg-gradient-to-r from-cyan-500 to-fuchsia-600 text-white font-bold py-2 px-6 rounded-lg shadow-lg hover:shadow-xl hover:from-cyan-600 hover:to-fuchsia-700 transform hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center">
                {isLoading && !isSavingPdf ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div> : 'Generate'}
            </button>
